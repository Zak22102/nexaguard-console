...
const CardGlow = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-lg border-glow-sm bg-card text-card-foreground transition-all duration-300 hover:border-glow",
        className
      )}
      {...props}
    />
...
export { Card, CardGlow, CardHeader, CardFooter, CardTitle, CardDescription, CardContent };
